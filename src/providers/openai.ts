/**
 * OpenAI provider: handles /v1/chat/completions endpoint logic
 */
...
/**
 * Handles OpenAI-compatible chat completion request body.
 * @param body - Request body (already parsed JSON)
 * @param res - HTTP response object
 */
export async function handleOpenAIRequest(body: unknown, res: ServerResponse): Promise<void> {
  state.activeRequests++;
  verbose(`OpenAI request started (active=${state.activeRequests})`);
  ...
  try {
    ...
    verbose(`LM request via OpenAI model=${model.family || model.id || model.name || 'unknown'} tools=${lmTools.length}`);
    ...
    try {
      ...
      try {
        if (chatContext.isStreaming) {
          await streamOpenAIResponse(res, response, chatContext);
        } else {
          const processed = await collectOpenAIData(response);
    ...
    state.activeRequests--;
    verbose(`OpenAI request complete (active=${state.activeRequests})`);
  }
}
...
async function streamOpenAIResponse(
  ...
  verbose(`OpenAI SSE start id=${context.requestId}`);
  ...
  res.write('data: [DONE]\n\n');
  res.end();
  verbose(`OpenAI SSE end id=${context.requestId}`);
}
 ...
 */
async function collectOpenAIData(
  response: vscode.LanguageModelChatResponse
...
function sendCompletionResponse(
  ...
  verbose(`OpenAI non-stream complete len=${data.content.length} tool_calls=${data.toolCalls.length}`);
